const Russian: typeof import('@locales/English').default = {
  ...
  ECONOMY: {
    ...
    BALANCE: "Баланс",
    BANK: "Банк",
    TIME_ERROR: "Вы уже получили свою награду!\nПопробуйте ещё раз через {time}",
    ...
    CASES: {
      ...
      GOLD: "Золотой Кейс",
      CHOOSE_TEXT: "Для продолжения, выберите один из предложенных кейсов!",
  ...
  GAMES: {
    CAPTCHA: {
      ...
      CORRECT_ANSWER: "Ваш ответ оказался верным, вы получили {coins} Коинов на ваш Баланс!",
    ...
    GUESS_THE_LOGO: {
      DESCRIPTION: "Угадайте Логотип чтобы получить {reward} коинов!\n\n› Зацепка: {clue}\n› Подсказка: {hint}",
  ...
  MODERATION: {
    ...
    TEMPMUTED: "Успешно Временно Замутил {target}!\nВремя: {time}\nПричина: {reason}\nМодератор: {moderator}",
  ...
  MUSIC: {
    ...
    VOLUME_SETTED: volume => `Текущая громкость песни изменена на ${volume}%`
  },
  OTHER: {
    BOTINFO: {
      TITLE: "Статистика",
  ...
  SETTINGS: {
    CONFIG: {
      ...
      WIKI: "¹ - Данный канал предназначен для Уведомлений об Участниках (их заходах и уходах с сервера)"
    },
    SETTED: (type, value) => `Успешно изменил "${type}" на "${value}"`,
    RESETTED: (type, value?) => `Успешно сбросил "${type}" на изначальное значение${value !== undefined ? ` (${value})` : ''}`,
    ...
    LANGUAGE_CHANGED_NOTE: "Если вы сделали это по-случайности, используйте команду 'language-reset'"
  },
  ERRORS: {
    NO_ACCESS: "У вас нет доступа к данной команде!",
    MEMBER_MISSINGPERMS: "Вы не имеете следующие права для запуска команды: {perms}",
    BOT_MISSINGPERMS: "У меня нет следующих прав для запуска команды: {perms}",
    ARGS_MISSING: "Вы упустили важный аргумент!\nВоспользуйтесь командой \"help {cmd_name}\" для получения примера использования!",
    EVAL_CANCELED: "Процесс обработки кода досрочно отменён, так как ответ может содержать личную информацию!",
    ...
    MEMBER_NOT_BANNABLE: "Не могу забанить {target} потому, что Участник имеет Иммунитет от этого!",
    MEMBER_NOT_KICKABLE: "Не могу кикнуть {target} потому, что Участник имеет Иммунитет от этого!",
    NO_MUTEROLE: "Для использования команды, серверу необходимо иметь Мут Роль!",
    CLEAR_LIMIT: "Бот может удалять максимум 100 сообщений за использование!",
    NO_MUTE: "{member} не имеет активного мута!",
    NO_WARNS: "{member} не имеет предупреждений!",
    NOT_JOINED_VOICE: "Для использования командой, вам необходимо зайти в Голосовой Канал!",
    JOIN_BOT_VOICE: "Для использования командой, вам необходимо зайти в Голосовой Канал со мной!",
    ...
    MISSING_IN_LIST: (value, list) => `"${value}" не найден в списке!\nВыборы: ${list}`,
  ...
  EVENTS: {
    ...
    GUILD_EVENTS: {
      ...
      ROLE_ADD: {
        ...
        DESCRIPTION: "{member} получил роль {role}!\nМодератор: {moderator}"
      },
      ROLE_REMOVE: {
        TITLE: "⬇️ | Участник лишился роли!",
        DESCRIPTION: "{member} лишился роли {role}!\nМодератор: {moderator}"
      },
      ...
      VERIFIED: {
        ...
        DESCRIPTION: "Сервер только что получил статус \"Верифицированный\"!"
      },
      UNVERIFIED: {
        TITLE: "😔 | Server got UnPartnered!",
        DESCRIPTION: "Сервер только что потерял статус \"Верифицированный\"!"
      }
    },
    LEVELING: {
