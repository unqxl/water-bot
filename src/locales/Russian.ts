const Russian: typeof import("@locales/English").default = {
	BOTOWNER: {
		...
		COMMAND_RELOADED: (name) => `Команда с названием "${name}" успешно перезагружена!`
	},
	...
	OTHER: {
		...
		SOURCE: {
			TEXT: (url) => `С недавних пор, исходный код бота доступен в моём репозитории на GitHub: ${url}`
		}
	},
	...
	EVENTS: {
		...
		MESSAGE_EVENTS: {
			DELETE: {
				...
				DESCRIPTION: "Сообщение от {author} было удалено!\n\n› Содержание сообщения: `{content}`\n› Дата удаления: {date}",

				GHOST_PING: {
					TITLE: "⚠️ | Обнаружено Упоминание",
					DESCRIPTION: "Похоже, что сообщение от {author} имеет упоминания!\n\n› Содержание сообщения: `{content}`",
				}
			},
			UPDATE: {
