const Russian: typeof import("@locales/English").default = {
	...
	ROLEPLAY: {
		ACTIONS: {
			...
			SLAP: (author, target) => `${author} ударил(а) ${target}!`,
	...
	EVENTS: {
		...
		HAPPEND_AT: (date) => `Произошло: ${date}`,

		GUILD_BIRTHDAY: {
			TEXT: (name, years, decl) =>
				`🎉 | "${name}" празднует свой день рождения!\nСерверу уже ${years} ${decl}!`,
			YEAR: "год",
			YEARS: "лет",
		},
		...
		GUILD_EVENTS: {
			...
			MEMBER_ADD: {
				...
				DESCRIPTION: (member) =>
					`${member} просто зашёл на этот сервер!`,
			},
			MEMBER_REMOVE: {
				TITLE: "⬇️ | Участник покинул сервер!",
				DESCRIPTION: (member) =>
					`${member} просто покинул этот сервер!`,
			},

			//guildMemberBoost | guildMemberUnboost
			MEMBER_BOOST: {
				TITLE: "🎉 | Участник забустил сервер!",
				DESCRIPTION: (member, boosts) =>
					`${member} забустил этот сервер!\nБустов: ${boosts}`,
			},
			MEMBER_UNBOOST: {
				TITLE: "😔 | Участник разбустил сервер!",
				DESCRIPTION: (member, boosts) =>
					`${member} разбустил этот сервер!\nБустов: ${boosts}`,
			},
			...
			ROLE_ADD: {
				...
				DESCRIPTION: (member, role, moderator) =>
					`${member} получил роль ${role} модератором ${moderator}`,
			},
			ROLE_REMOVE: {
				TITLE: "⬇️ | Участник потерял роль!",
				DESCRIPTION: (member, role, moderator) =>
					`${member} потерял роль ${role} модератором ${moderator}`,
			},

			//guildBoostLevelUp | guildBoostLevelDown
			LEVEL_UP: {
				TITLE: "🎉 | Уровень буста повышен!",
				DESCRIPTION: (new_level) =>
					`Данный сервер получил новый ${new_level} уровень буста!`,
			},
			LEVEL_DOWN: {
				TITLE: "😔 | Уровень буста понижен!",
				DESCRIPTION: (new_level) =>
					`Уровень буста сервера понизился до ${new_level}!`,
			},
			...
			PARTNERED: {
				...
				DESCRIPTION:
					'Данный сервер только что получил статус "Партнёрский"!',
			},
			UNPARTNERED: {
				TITLE: "😔 | Сервер потерял Партнёрку!",
				DESCRIPTION:
					'Данный сервер только что потерял статус "Партнёрский"!',
			},
			...
			VERIFIED: {
				...
				DESCRIPTION:
					'Данный сервер только что получил статус "Верифицированный"!',
			},
			UNVERIFIED: {
				TITLE: "😔 | Сервер потерял Верификацию!",
				DESCRIPTION:
					'Данный сервер только что потерял статус "Верифицированный"!',
			},

			//guildBanAdd | guildBanRemove
			BAN_ADD: {
				TITLE: "🛡️ | Пользователь получил бан!",
				DESCRIPTION: (member, tag, moderator, reason) =>
					`${member} (${tag}) был забанен модератором ${moderator}\n› Причина: ${reason}`,
			},
			BAN_REMOVE: {
				TITLE: "🛡️ | Пользователь получил разбан!",
				DESCRIPTION: (member, tag, moderator, reason) =>
					`${member} (${tag}) был разбанен модератором ${moderator}\n› Причина бана: ${reason}`,
			},
		},

		LEVELING: {
			NEWLEVEL: (user, level) =>
				`🎉 | ${user} повысил уровень на ${level}`,
		},
		...
		MESSAGE_EVENTS: {
			DELETE: {
				...
				DESCRIPTION: (author, content) =>
					`Сообщение от ${author} было удалено!\n\n› Содержание сооьщения: \`${content}\``,

				GHOST_PING: {
					TITLE: "⚠️ | Обнаружено упоминание!",
					DESCRIPTION: (author, content) =>
						`Похоже, что сообщение от ${author} содержит упоминание участника/роли!\n\n› Содержание сообщения: \`${content}\``,
				},
			},
			UPDATE: {
				TITLE: "⬆️ | Сообщение обновлено!",
				DESCRIPTION: (author, old_content, new_content) =>
					`Сообщение от ${author} было обновлено!\n\n› Содержание до изменения: \`${old_content}\`\n› Содержание после изменения: \`${new_content}\``,
				GO_TO: "Перейти",
			},
		},
