...
export = async (
  ...
  const description = lang.GAMES.PHASMOPHOBIA.WELCOME.replace('{type}', ghostType);
      ...
      collector.on("collect", async (collected: ButtonInteraction) => {
        switch (collected.customId) {
          case "success": {
            ...
            client.economy.balance.add(500, msg.author.id, message.guild.id);
          ...
          case "fake":
          case "fake_1": {
            const failure = lang.GAMES.PHASMOPHOBIA.DEFEAT.replace('{correct}', successEvidences.join(', '));
            const embed = client.functions.buildEmbed(
              msg,
              "BLURPLE",
              bold(failure),
