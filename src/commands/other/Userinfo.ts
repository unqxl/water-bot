...
export default class UserinfoCommand extends Command {
  ...
  async run(message: Message, args: string[], lang: typeof import('@locales/English').default) {
    ...
    const userInfo = {
      ...
      regTimeAgo: timeSince(this.client, message, member.user.createdTimestamp),
      joinTimeAgo: timeSince(this.client, message, member.joinedTimestamp),
      ...
      presence: () => {
        ...
        if (member.presence.status === "offline") return `⚫ ${offline}`;
        if (member.presence.status === "invisible") return `⚫ ${offline}`;
      },
      ...
      presenceGame: () => {
        ...
        if (
          ...
          return `${member.presence.activities.join(", ").toString()}`;
        } else {
          return notPlaying;
        }
      },
      onlineUsing: () => {
        ...
        if (
          ...
          if (desktop && web) content = `${status_desktop}, ${status_web}`;
          if (desktop && mobile)
            content = `${status_desktop}, ${status_mobile}`;
          if (mobile && web) content = `${status_mobile}, ${status_web}`;
          if (mobile && web && desktop)
            content = `${status_desktop}, ${status_web}, ${status_mobile}`;
          if (web && mobile) content = `${status_web}, ${status_mobile}`;
