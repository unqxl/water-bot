...
export default class AddLevelCommand extends Command {
    ...
    async validate(message: Message, args: string[], lang: typeof import('@locales/English').default): Promise<ValidateReturn> {
        ...
        if (!member) {
            const text = lang.ERRORS.ARGS_MISSING.replace('{cmd_name}', 'addlevel');
    ...
    async run(message: Message, args: string[], lang: typeof import('@locales/English').default) {
        ...
        const text = lang.LEVELING.ADDED_LEVEL.replace('{level}', this.client.functions.sp(amount)).replace('{target}', member.toString());
