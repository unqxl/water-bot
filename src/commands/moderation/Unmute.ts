...
export default class UnmuteCommand extends Command {
  ...
  async validate(
    ...
    if (!member) {
      const text = lang.ERRORS.ARGS_MISSING.replace('{cmd_name}', 'unmute');
    ...
    if (!lastMute) {
      const text = lang.ERRORS.NO_MUTE.replace('{member}', member.toString());
      const embed = this.client.functions.buildEmbed(
        message,
        "BLURPLE",
        bold(text),
  ...
  async run(message: Message, args: string[], lang: typeof import('@locales/English').default) {
    ...
    collector.on("collect", async (btn: ButtonInteraction) => {
      if (btn.customId === "confirm") {
        ...
        const text = lang.MODERATION.UNMUTED.replace('{target}', member.toString());
        const embed = this.client.functions.buildEmbed(
          message,
          "BLURPLE",
          bold(text),
