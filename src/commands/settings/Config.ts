  ...
  async run(message: Message, args: string[], lang: typeof import('@locales/English').default) {
    ...
    const streamers =
      config.twitchStreamers.length !== 0
        ? config.twitchStreamers.map((streamer) => streamer.name).join(", ")
        : "-";

    var text = "";
    text += `› ${bold(language)}: ${bold(config.language)}\n\n`;
    text += `› ${bold(memberschannel)}: ${bold(membersChannel.toString())}\n`;
    text += `› ${bold(logchannel)}: ${bold(logChannel.toString())}\n`;
    text += `› ${bold(levelschannel)}: ${bold(levelsChannel.toString())}\n`;
    text += `› ${bold(twitchchannel)}: ${bold(twitchChannel.toString())}\n\n`;
