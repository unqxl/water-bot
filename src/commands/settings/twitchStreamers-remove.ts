...
export default class TwitchStreamersRemoveCommand extends Command {
    ...
    async validate(message: Message, args: string[], lang: typeof import('@locales/English').default): Promise<ValidateReturn> {
      ...
      if (!channel) {
        const text = lang.ERRORS.ARGS_MISSING.replace('{cmd_name}', 'twitchstreamers-remove');
        const embed = this.client.functions.buildEmbed(
          message,
          "BLURPLE",
          bold(text),
    ...
    async run(message: Message, args: string[], lang: typeof import('@locales/English').default) {
      ...
      const streamer = streamers.find((streamer) => streamer.name === channel);

      if(!streamer) {
        const text = lang.ERRORS.MISSING_IN_LIST(channel, streamers.map((x) => x.name).join(', '));
