...
export default class CaptchaCommand extends Command {
    ...
    async run(message: Message, args: string[], lang: typeof import('@locales/English').default) {
        ...
        const text = lang.GAMES.CAPTCHA.TEXT.replace('{reward}', this.client.functions.sp(reward));
        const embed = this.client.functions.buildEmbed(message, 'BLURPLE', bold(text), false, true);
        ...
        if(content !== answer) {
            ...
            const text = lang.GAMES.CAPTCHA.CORRECT_ANSWER.replace('{coins}', this.client.functions.sp(reward));
            const embed = this.client.functions.buildEmbed(message, 'BLURPLE', bold(text), '✅', true);
