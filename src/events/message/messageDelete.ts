...
export const run: RunFunction = async(client, message: Message) => {
    ...
    const logChannelID = client.database.getSetting(message.guild, 'logChannel');
    ...
    const [ title, description ] = [
        ...
        lang.EVENTS.MESSAGE_EVENTS.DELETE.DESCRIPTION
        .replace("{author}", message.author.toString())
        .replace("{executor}", audit ? audit.executor.toString() : message.author.toString())
