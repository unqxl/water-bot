import { Message, MessageEmbed, TextChannel, MessageButton, MessageActionRow } from 'discord.js';
import { RunFunction } from '../../interfaces/Event';
import { bold } from '@discordjs/builders';

export const name: string = 'messageUpdate';

export const run: RunFunction = async(client, oldMSG: Message, newMSG: Message) => {
    ...
    const logChannel = newMSG.guild.channels.cache.get(logChannelID) as TextChannel;
    ...
    const [ title, description, redirectText ] = await Promise.all([
        ...
        lang.EVENTS.MESSAGE_EVENTS.UPDATE.DESCRIPTION
        .replace("{author}", newMSG.author.toString())
