...
export const run: RunFunction = async (client, message: Message) => {
  ...
  const levelData = client.levels.getData(message.guild.id, message.author.id);
  ...
  if (mentionCheck) {
    const text = lang.EVENTS.GUILD_PREFIX.replace('{guild}', Util.escapeMarkdown(message.guild.name)).replace('{prefix}', prefix);
  ...
  if (command.options.memberPermissions) {
    if (
      !channel.permissionsFor(message.member).has(command.options.memberPermissions)
    ) {
      const perms = channel
      ...
      if (perms.length > 0) {
        var missing = "";
        perms.map((perm) => (missing += lang.PERMISSIONS[perm])).join(", ");

        const text = lang.ERRORS.MEMBER_MISSINGPERMS.replace('{perms}', missing);
        const embed = client.functions.buildEmbed(
          message,
          "BLURPLE",
          bold(text),
  ...
  if (command.options.botPermissions) {
    if (
      ...
      if (perms.length > 0) {
        ...
        const text = lang.ERRORS.BOT_MISSINGPERMS.replace('{perms}', missing);
        const embed = client.functions.buildEmbed(
          message,
          "BLURPLE",
          bold(text),
