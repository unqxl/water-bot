...
export default class GuildBoostLevelUpEvent extends Event {
	constructor() {
		super("guildBoostLevelUp");
	}
	...
	async run(client: Bot, guild: Guild, oldLevel: number, newLevel: number) {
		...
		const title = lang_file.EVENTS.GUILD_EVENTS.LEVEL_UP.TITLE;
		const description =
			lang_file.EVENTS.GUILD_EVENTS.LEVEL_UP.DESCRIPTION(newLevel);
