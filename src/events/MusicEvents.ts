...
export = async (client: Goose) => {
  return client.music
    .on("addSong", async (queue, song) => {
      const lang = await client.functions.getLanguageFile(queue.textChannel.guild);
      const [embed_title, title, url, views, duration, songs, requestedBy] = [
      ...
      text += `› ${bold(views)}: ${bold(client.functions.sp(song.views))}\n`;
    ...
    .on("addList", async (queue, playlist) => {
      const lang = await client.functions.getLanguageFile(queue.textChannel.guild);
      const [embed_title, title, url, views, duration, playlistSongs, songs, requestedBy] = [
      ...
      text += `› ${bold(url)}: ${bold(hyperLink)}\n`;
      text += `› ${bold(views)}: ${bold(client.functions.sp(playlist.views))}\n`;
      text += `› ${bold(duration)}: ${bold(playlist.formattedDuration)}\n`;
      ...
      text += `› ${bold(requestedBy)}: ${bold(playlist.user.toString())}\n`;
